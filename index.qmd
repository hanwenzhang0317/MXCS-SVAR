...
# quarterly, seasonally adjusted, total taxes, unit: $M A2302794K
# 1959-09-01 to 2022-12-01 --> 254 quarters 
total_taxes <- readabs::read_abs(series_id = "A2302794K")
tmp.total_taxes <- xts::xts(total_taxes$value, total_taxes$date)
total_taxes_pc <- tmp.total_taxes / pop / tmp.gdp.deflator

# original CPI
# 1948-09-01 to 2022-12-01 --> 298 quarters 
cpi <-  readabs::read_abs(series_id = "A2325846C") 
tmp.cpi <- xts::xts(cpi$value, cpi$date) 
inflation <- 100*diff(log(tmp.cpi))
...
df.o <- as.data.frame(
  merge(tmp.gdp_no, # Nominal GDP $M
            ...
            tmp.total_taxes, # Total taxation $M
...
var.names <- c("Nominal GDP $M","Government Spending $M","Total Taxes $M","Pubic Debt $M","Inflation (%)","Cash Rate Target (%)","Stock Prices $")
# col <- c("#009999","#009999", "#009999", "#009999", "#9933CC", "#9933CC", "maroon")
col <- c(RColorBrewer::brewer.pal(9, name = "Purples"))

par(mfrow=c(4,2), mar=c(2,2,2,2))
for (i in 1:ncol(df.o)){
  plot(dates, y = df.o[,i], type = "l", 
       main = paste(var.names[i]), ylab = "", xlab = "",
       col = col[i+2], lwd = 2.5,
...
Variables are transformed into real per-capita terms and compute logarithms as in @Mumtaz2020stock, except inflation and cash rate target. In this stage, only the sample including all variables ranging from 2001 Q2 to 2022 Q4 is considered in the following section. Plots are provided as @fig-line-plot.
...
colnames(df.log) <- c("GDP","Spending","Taxation","Debt","Inflation")
df.log$Interest <- as.numeric(as.character(interest))
df.log$Stock <- log(as.numeric(aord))[1:132]
```
...
var.names <- c("GDP $M","Government Spending $M","Total Taxes $M","Pubic Debt $M","Inflation (%)","Cash Rate Target (%)","Stock Prices $")
...
ACF Plots as in @fig-acf-plot show non-zero autocorrelations for at least 15 lags of  variables except Inflation. PACF Plots as @fig-pacf-plot indicate significant and strong memory at the first lag of variables except Inflation, for which the first lag is also statistically significant but not strong.
...
for (j in 1:length(colnames(df))){
  ...
  title(main = paste(var.names[j]), line = 1)
}
```
...
Augmented Dickey-Fuller Test can be performed to test for stationarity under the null hypothesis of unit-root non-stationary, the results are shown as @tbl-adf, from which we do not reject the null and conclude that all variables except for cash rate target are unit-root non-stationary.
...
Take the first difference and run the Augmented Dickey-Fuller Test again, it is sensible to conclude that variables except public debt and cash rate target are integrated of order 1 as in @tbl-adf-diff. Public debt is integrated with order 2, since its second-order difference is stationary as @tbl-adf-diff2.
...
The Algorithm from @Fry2011effects is implemented to transfer SF parameters $(\tilde{B_+},\tilde{B_0})$ to parameters $(B_{+},B_{0})$ such the restrictions of interest holds.
...
Considering potential existence of heteroskedasticity in the variables I used, Stochastic Volatility (SV) models can be estimated, for which the specification would be: 
\begin{gather}
Y = XA + E \\
\\ E|X \sim MN_{T \times N}(0_{T \times N},\Sigma,\text{diag}(\sigma^2)) 
\end{gather} 
where $\sigma^2=(\sigma^2_1,...,\sigma^2_T)^{'}$, which follows a Stochastic Volatility process. 
...
1.  Draw $(A,\Sigma)^{(s)} \sim p(A,\Sigma| X,Y,{\sigma^2}^{(s-1)})$
...
```{r}
prior.parameters.sv <- GetPrior.parameters(N,p,Sigma.hat)
posterior.draws <-
DrawPosterior.sv(Y,X,prior.parameters.sv,N_S1,N_S2)
```

@fig-pos-svts provides a time series plot of posterior draws mean of $\sigma^2$ at each time spot. From where we can observe the $\sigma^2$ oscillating around 2400. 

```{r}
#| label: fig-pos-svts 
#| fig-cap: "Posterior Draw Mean of Stochastic Volatility "
sigma2.mean.vec <- apply(posterior.draws$H.sv,1,mean) 
...
@fig-Gibbs-Sampler-SV plots the S2 draws of Gibbs Sampler of the Stochastic Volatility model, and it shown to be stationary for A and $\Sigma$ when $S_1=5000, S_2=45000$.From where, we can observe the mean of $A_{21},A_{32}$ varying around value of 1, $\Sigma_{21},\Sigma_{22}$ oscillating around 0.0018-0.0019. Under the existence of stochastic volatility, it does not make sense to check whether the posterior draw of $\Sigma$ matrix converges to Identity matrix, but we might conclude that, together with $\sigma^2$, we have reasonable estimated level of the variance matrix of the SVAR model.     

```{r}
#| label: fig-Gibbs-Sampler-SV 
#| fig-cap: "Gibbs Sampler draws of A and Sigma-SV model"
par(mfrow=c(2,2),mar=c(2,2,2,2))
plot(posterior.draws$A.posterior[2,1,],type='l',col="#660099",ylab="",xlab="",main=expression(A[21]), lwd = 0.1)
plot(posterior.draws$A.posterior[3,2,],type='l',col="#CC66CC",ylab="",xlab="",main=expression(A[32]), lwd = 0.1)
plot(posterior.draws$Sigma.posterior[1,1,],type='l',col="#6699CC",ylab="",xlab="",main=expression(Sigma[21]), lwd = 0.1)
plot(posterior.draws$Sigma.posterior[2,2,],type='l',col="#0066CC",ylab="",xlab="",main=expression(Sigma[22]), lwd = 0.1)
```
...
Under this setting, we consider the following posterior distributions:
\begin{gather}
p(\kappa|A,\Sigma,Y,X) \\ 
\\ p(A,\Sigma|X,Y,\kappa) 
\end{gather}
...
@fig-Gibbs-Sampler plots the S2 draws of Gibbs Sampler, and it shown to be stationary for A and $\Sigma$. And it can be observed that the diagonal elements of $\Sigma$ matrix and sub-diagonal elements of $A$ converges to 1, which is supported by @tbl-Acheck2 and @tbl-Scheck2. 
...
@tbl-Acheck2 and @tbl-Scheck2 also shows desired results, suggesting the extended model is producing estimates close to true parameters of the data generating process.

```{r}
#| label: tbl-Acheck2
#| tbl-cap: Inverted Gamma 2 scale Model - A 
...
```{r}
#| label: tbl-Scheck2
#| tbl-cap: Inverted Gamma 2 scale Model - Sigma 
...
Following @Mountford2009effects, the government revenue shock is identified as a shock where government revenue rises for a year after the shock. In this project, I am restricting only the current period, where the sign of government spending and debt are positive, with all others not restricted.
...
@tbl-pos-A presented the mean of posterior draws of A matrices as in the reduced form. 
```{r}
#| label: tbl-pos-A
#| tbl-cap: Inverted Gamma 2 scale Model - A 
d3 <- round(apply(posterior.draws$A.posterior,1:2,mean),4)
colnames(d3) <- colnames(df)
knitr::kable(d3, index=TRUE)
```

@fig-pos-baseline plots the S2 draws of Gibbs Sampler of the baseline model, and it shown to be stationary for A and $\Sigma$.

```{r}
#| label: fig-pos-baseline
#| fig-cap: "Posterior draws of A and Sigma-Baseline model"
par(mfrow=c(2,2),mar=c(2,2,2,2))
plot(posterior.draws$A.posterior[2,1,],type='l',col="#660099",ylab="",xlab="",main=expression(A[21]), lwd = 0.1)
plot(posterior.draws$A.posterior[3,2,],type='l',col="#CC66CC",ylab="",xlab="",main=expression(A[32]), lwd = 0.1)
plot(posterior.draws$Sigma.posterior[1,1,],type='l',col="#6699CC",ylab="",xlab="",main=expression(Sigma[21]), lwd = 0.1)
...
@fig-IRF-basic shows the Impulse Response Function of the baseline model for 50,000 draws, from which we can't observe any significant or meaningful effects on variables of interest.

```{r}
#| label: fig-IRF-basic 
#| fig-cap: "Impulse Response Function-Basic Model"
names <- c("GDP","Government Spending","Total Taxes","Pubic Debt","Inflation","Cash Rate Target","Stock Prices")
...
for (n in 1:N){
  ...
  if (n==5){
    axis(1,c(1,5,9),c("0","1yr","2yr"))
  } else {
    axis(1,c(1,5,9),c("0","1 yr","2yr"))
  }
...
S2       <- N_S2 
prior.parameters <- GetPrior.parameters(N,p,Sigma.hat)
posterior.draws <- DrawPosterior.sv(Y,X,prior.parameters,S1,S2)
...
As presented in @fig-ts-svmean, the stochastic volatility mean plot has a significant difference with @fig-pos-sv, it exhibits higher volatility during AfC, GFC and Covid-19 pandemic. 

```{r}
#| label: fig-ts-svmean 
#| fig-cap: "Time Series Plot of Stochastic Volatility Mean"

sv.mean = apply(sqrt(posterior.draws$H.sv),1,mean)
...
@fig-pos-sv plots the S2 draws of Gibbs Sampler of the baseline model, and it shown to be stationary for A and $\Sigma$.
...
IRF.posterior.mps = IRF.posterior[,2,,]
IRFs.k1.sv           = apply(IRF.posterior.mps,1:2,median)

IRFs.k1.sv           = apply(IRF.posterior.mps,1:2,median)
IRFs.inf.k1       = apply(IRF.posterior.mps,1,mean)
rownames(IRFs.k1.sv) = colnames(df.num)

IRFs.k1.sv.hdi    = apply(IRF.posterior.mps,1:2,hdi, credMass=0.68)
hh             = 1:(h+1)
```

@fig-IRF-SV presents the Impulse Response Function of the Stochastic Volatility model, which suggest a slight decrease in GDP while government spending rises, decrease in inflation and Cash Rate Target and Total Taxes. From this plot, we can see that responses are estimated with more confidence, shown by narrower confidence intervals. 

```{r}
#| label: fig-IRF-SV  
#| fig-cap: "Impulse Response Function-SV model"
par(mfrow=c(4,2), mar=c(2,2,2,2),cex.axis=1, cex.lab=1)

for (n in 1:N){
  ylims     = range(IRFs.k1.sv[n,hh],IRFs.k1.sv.hdi[,n,1:(h+1)],0)
  plot(hh,IRFs.k1.sv[n,hh], type="l", ylim=ylims, axes=FALSE, xlab="", main=names[n])
  if (n==5){
    axis(1,c(1,5,9),c("0","1yr","2yr"))
  } else {
    axis(1,c(1,5,9),c("0","1 yr","2yr"))
  ...
  axis(2,c(ylims[1],0,ylims[2]),round(c(ylims[1],0,ylims[2]),3))
  polygon(c(hh,(h+1):1), c(IRFs.k1.sv.hdi[1,n,hh],IRFs.k1.sv.hdi[2,n,(h+1):1]), col="#CBC9E2",border="#756BB1")
  abline(h=0)
  lines(hh, IRFs.k1.sv[n,hh],lwd=2,col="black")
}
```
...
@fig-kappa-dist-em shows the distribution of Gibbs Sampler draws of $\kappa$.

```{r}
#| label: fig-kappa-dist-em
#| fig-cap: "Empirical Gibbs Sampler draws of kappa"
...
@fig-pos-ig2 plots the S2 draws of Gibbs Sampler of the inverted gamma 2 scale mixture, and it shown to be stationary for A and $\Sigma$.

```{r}
#| label: fig-pos-ig2
#| fig-cap: "Posterior draws of A and Sigma-Inverted Gamma 2 Scale Mixture"
par(mfrow=c(2,2),mar=c(2,2,2,2))
plot(posterior.draws$A.posterior[2,1,],type='l',col="#660099",ylab="",xlab="",main=expression(A[21]), lwd = 0.1)
plot(posterior.draws$A.posterior[3,2,],type='l',col="#CC66CC",ylab="",xlab="",main=expression(A[32]), lwd = 0.1)
plot(posterior.draws$Sigma.posterior[1,1,],type='l',col="#6699CC",ylab="",xlab="",main=expression(Sigma[21]), lwd = 0.1)
...
IRF.posterior.mps = IRF.posterior[,2,,]
IRFs.k1.ig           = apply(IRF.posterior.mps,1:2,median)
IRFs.k1.ig           = apply(IRF.posterior.mps,1:2,median)
IRFs.inf.k1       = apply(IRF.posterior.mps,1,mean)
rownames(IRFs.k1.ig) = colnames(df.num)

IRFs.k1.ig.hdi    = apply(IRF.posterior.mps,1:2,hdi, credMass=0.68)
hh             = 1:(h+1)
```

@fig-IRF-extended presents the Impulse Response Function of the extended model, which suggest a slight decrease in GDP while government spending rises, decreases in GDP and Cash Rate Target, with increase in stock prices. Compared to the baseline model, the change is not much pronounced, but we can observe a wider confidence interval on Inflation. 
...
for (n in 1:N){
  ylims     = range(IRFs.k1.ig[n,hh],IRFs.k1.ig.hdi[,n,1:(h+1)],0)
  plot(hh,IRFs.k1.ig[n,hh], type="l", ylim=ylims, axes=FALSE, xlab="", main=names[n])
  if (n==5){
    axis(1,c(1,5,9),c("0","1yr","2yr"))
  } else {
    axis(1,c(1,5,9),c("0","1 yr","2yr"))
  ...
  axis(2,c(ylims[1],0,ylims[2]),round(c(ylims[1],0,ylims[2]),3))
  polygon(c(hh,(h+1):1), c(IRFs.k1.ig.hdi[1,n,hh],IRFs.k1.ig.hdi[2,n,(h+1):1]), col="#CBC9E2",border="#756BB1")
  abline(h=0)
  lines(hh, IRFs.k1.ig[n,hh],lwd=2,col="black")
}
```
...
# Conclusion 
In this project, I have applied Bayesian Estimation Approach to discover the effect of government spending shock on Australian Economy and stock market. A structural vector auto-regression model is applied and identified using sign restrictions. Models to incorporate stochastic volatility and inverted gamma 2 scaled priors are also applied. Since all the models converge under the artificial data and real data, we might suggest that they are correctly estimated. From the impulse response function plots, a government spending shock seems to not have significant influence on the aggregate economy, under the background of Australia. It might be augured that business cycle shocks and monetary shocks are not controlled as in @Mountford2009effects, and sign restrictions might need to be applied in a more complicated system. 
