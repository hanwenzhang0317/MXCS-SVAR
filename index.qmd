...
```{r}
# GetPrior.parameters.e <- function (N,p,Sigma.hat,nu.kappa,S.kappa) {
#  
...
GetPoesterior.parameters.e <- function (X,Y,A.prior,nu.prior,S.prior,V.prior) {
...
```{r}
DrawPosterior.e <- function (N,S,p,nu.bar,S.bar,A.bar,V.bar){
  
  ASigma.posterior   = rWishart(S, df=nu.bar, Sigma=solve(S.bar))  
  Sigma.posterior   = apply(Sigma.posterior,3,solve)            
  Sigma.posterior   = array(Sigma.posterior,c(N,N,S))   
  A.posterior       = array(rnorm(prod(c(dim(A.bar),S))),c(dim(A.bar),S)) 
  B.posterior       = array(NA,c(N,N,S))
