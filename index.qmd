...
Then posterior distributions can be drawn: 
...
After the posterior draws,we apply sign restrictions to  
...
In the extended model, instead of setting $\kappa_1$ and $\kappa_2$ as fixed values, we can assume $\kappa_1$ is from an inverse gamma 2 distribution as

\begin{align}
\kappa \sim IG2(\underline{S}_ \kappa,\underline{\nu}_ \kappa)
\end{align}

$\kappa_2$ can be set as $100 \times \kappa_1$. 
...
```{r}
DrawPosterior.e <- function (N,p,Y,X,nu.prior,A.prior,V.prior,S.prior,nu.kappa,S.kappa,S1,S2){
  ...
  for (s in 1:(S1+S2)){
    ...
    draw.sigma = apply(rWishart(1, df=nu.bar, Sigma=solve(S.bar)),3,solve)
    Sigma.posterior[,,s] = array(draw.sigma,c(N,N))
    
    cholSigma.s            = chol(Sigma.posterior[,,s])
    A.posterior[,,s]       = matrix(MASS::mvrnorm(1,as.vector(A.bar),Sigma.posterior[,,s]%x%V.bar),ncol=N)               # from lec10 - page 12 
    ...
    S.kappa.bar <- S.kappa + sum(diag( solve(Sigma.posterior[,,s]) %*% t(A.posterior[,,s]-A.prior)%*% solve(V.prior) %*% (A.posterior[,,s]-A.prior)))
...
## Artificial Data Check

1000 observations of bi-variate Gaussian random walk processes with the covariance matrix equal to the identity matrix of order 2 are generated as @fig-actificial-data  to test the model framework.
...
prior.parameters = GetPrior.parameters.g(N,p,Sigma.hat,nu.kappa,S.kappa,FALSE)
...
@tbl-Acheck Shows the rotation matrix 
```{r}
#| label: tbl-Acheck
...
prior.e <- GetPrior.parameters.g(N,p,Sigma.hat,nu.kappa,S.kappa,TRUE)

A.prior <- prior.e$A.prior
V.prior <- prior.e$V.prior
S.prior <- prior.e$S.prior
nu.prior <- prior.e$nu.prior
kappa <- prior.e$kappa
```
...
S1 = 1000
S2 = 9000
posterior.e <- DrawPosterior.e(N,p,Y,X,nu.prior,A.prior,V.prior,S.prior,nu.kappa,S.kappa,S1,S2)

Sigma.posterior <- posterior.e$Sigma.posterior.e
A.posterior <- posterior.e$A.posterior.e
B1.tilde.s <- posterior.e$B1.tilde.s.e
B.posterior <- posterior.e$B.posterior.e

```

```{r apply sign restriction-extended model}
B.draws.restricted <- ImposeSignRestriction(restrictions, N,S2,p,A.posterior,Sigma.posterior,B.posterior,B1.tilde.s)
