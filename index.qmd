...
\\ \underline{V} = diag([\kappa_2 \quad \kappa_1 (p^{-2} \otimes \imath_N)]) \\
...
Following @Mountford2009effects, the governemnt revenue shock is identified as a shock where govenemnt revenue rises for a year after the shock. In this project, I am restricting only the current period, where the sign of government spending and debt are positive, with all others not restricted.
...
@fig-IRF-basic shows the Impulse Response Function of the basic model for 50,000 draws, from which we can't observe any significant or meaningful effects on variables of interest.
...
@fig-IRF-extended presents the Impulse Response Function of the extended model, which suggest a slight decrease in GDP while government spending rise, decrease in inflation and Cash Rate Target and Stock Prices. Note that since the government revenue data is not seasonally adjusted, the density looks containing some seasonality.  
